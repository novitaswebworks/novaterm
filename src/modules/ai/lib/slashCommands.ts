...
Use grep/glob/list_directory/read_file to explore. Cap NOVATERM.md under 200 lines. Use write_file to create it (will go through normal approval).`;
...
export const NOVATERM_CMD_RE =
  /^<novaterm-command\s+name="([a-z0-9-]+)"(?:\s+state="([a-z]+)")?\s*\/>(?:\n+|$)/;

export function wrapWithCommandMarker(prompt: string, name: string): string {
  return `<novaterm-command name="${name}" />\n\n${prompt}`;
}
