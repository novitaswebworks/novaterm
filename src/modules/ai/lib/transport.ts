...
const NOVATERM_MD_MAX_BYTES = 32 * 1024;
...
async function readNovaTermMd(
  ...
  try {
    ...
    const content =
      r.content.length > NOVATERM_MD_MAX_BYTES
        ? r.content.slice(0, NOVATERM_MD_MAX_BYTES)
        : r.content;
...
export function createContextAwareTransport(deps: Deps) {
  const run = async (options: SendOptions) => {
    ...
    const projectMemory = await readNovaTermMd(live.workspaceRoot);
