...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-novaterm-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-novaterm-slot", String(slots.length));
  ...
  if (!fast) {
    ...
    if (p.snapshot) {
      try {
        ...
        console.warn("[novaterm] snapshot replay failed:", e);
      }
    }
    if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[novaterm] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function disposeSlot(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[novaterm] slot dispose failed:", e);
  }
  slot.host.remove();
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[novaterm-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[novaterm-webgl] dispose failed:", e);
  }
  try {
    const r = (
