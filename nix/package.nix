...
let
  ...
  srcMap = {
    x86_64-linux = fetchurl {
      url = "https://github.com/my-org/novaterm-ai/releases/download/v${version}/NovaTerm_${version}_amd64.deb";
      hash = sources.hashes.x86_64-linux;
    };
    x86_64-darwin = fetchurl {
      url = "https://github.com/my-org/novaterm-ai/releases/download/v${version}/NovaTerm_x64.app.tar.gz";
      hash = sources.hashes.x86_64-darwin;
    };
    aarch64-darwin = fetchurl {
      url = "https://github.com/my-org/novaterm-ai/releases/download/v${version}/NovaTerm_aarch64.app.tar.gz";
...
assert lib.assertMsg (builtins.hasAttr sys srcMap)
  "novaterm: unsupported platform ${sys}";
...
stdenv.mkDerivation {
  ...
  installPhase = if stdenv.hostPlatform.isLinux then ''
    ...
    install -Dm755 usr/bin/novaterm $out/bin/novaterm

    wrapProgram $out/bin/novaterm \
      "''${gappsWrapperArgs[@]}" \
